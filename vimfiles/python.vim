...
" Wrapping and tabs.
set tw=98 ts=4 sw=4 sta et sts=4 ai
...
set formatoptions=tq textwidth=99 foldignore= wildignore+=*.py[co]
...
def SetBreakpoint():
    ...
    vim.current.buffer.append(
       "%(space)simport pdb; pdb.set_trace() %(mark)s" %
         {'space':strWhite, 'mark': ' ### break here'}, nLine - 1)
...
def RemoveBreakpoints():
    ...
    for strLine in vim.current.buffer:
        if strLine.lstrip().startswith("import pdb; pdb.set_trace()  ### break here"):
            nLines.append( nLine)
        nLine += 1
